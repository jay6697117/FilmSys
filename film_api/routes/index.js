...
router.get('/api/getMovieList', function (req, res) {
  ...
  conn.query(sqlStr, (error, result, field) => {
    if (error) {
      ...
      console.log(`result.length 000:`, result.length);
      result = JSON.parse(JSON.stringify(result));
      if (result.length) {
        result = result.filter(value => {
          return new Date(value.show_date+','+value.show_time)-new Date()>0;
        });
        console.log(`result.length 001:`, result.length);
  ...
  conn.query(sqlStr, [userId, movieId], (error, result, field) => {
    if (error) {
      ...
      if (result[0]) {
        ...
        conn.query(
          sqlStr,
          [score, commentContent, commentDate, 0, 0, undefined, result[0].comment_id],
          (error, result, field) => {
            if (error) {
...
router.post('/api/order', function (req, res) {
  let { userId, scheduleId, orderPhone, orderDate, ticketNum, totalPrice, orderSeatInfo, payType } = req.body;
  ...
  conn.query(
    sqlStr,
    [userId, scheduleId, orderPhone, orderDate, ticketNum, totalPrice, orderSeatInfo, payType, phoneCode],
    (error, result, field) => {
      if (error) {
        console.log(error);
      } else {
...
router.get('/api/getOrderByUserId', function (req, res) {
  let userId = req.query.userId;
  let sqlStr =
    'SELECT * FROM t_order INNER JOIN t_schedule ON t_order.schedule_id = t_schedule.schedule_id INNER JOIN t_movie ON t_movie.movie_id = t_schedule.movie_id INNER JOIN t_cinema ON t_cinema.cinema_id = t_schedule.cinema_id WHERE user_id = ? ORDER BY order_id DESC;';
  conn.query(sqlStr, [userId], (error, result, field) => {
    if (error) {
      console.log(error);
    } else {
...
router.post('/api/admin/updateUserInfo', function (req, res) {
  let { userId, userName, avatar, password, sex, phone, sign, birthday } = req.body;
  if (userId) {
    let sqlStr = 'SELECT * from t_user WHERE user_id = ? LIMIT 1;';
    conn.query(sqlStr, [userId], (error, result, field) => {
      if (error) {
              ...
              conn.query(sqlStr, [phone, userId], (error, result, field) => {
                if (error) {
                  ...
                  if (result[0]) {
                    ...
                    conn.query(
                      sqlStr,
                      [userName, avatar, password, sex, phone, birthday, sign, userId],
                      (error, result, field) => {
                        if (error) {
                          res.json({ error_code: 1, message: '更新用户信息失败' });
                          console.log(error);
...
router.post('/api/admin/updateMovieInfo', function (req, res) {
  let { movieId, movieName, poster, director, actor, long, type, language, publicDate, intro } = req.body;
  ...
  conn.query(sqlStr, [movieName, movieId], (error, result, field) => {
    if (error) {
      ...
      if (result[0]) {
        ...
        conn.query(
          sqlStr,
          [movieName, poster, director, actor, long, type, language, publicDate, intro, movieId],
          (error, result, field) => {
            if (error) {
              console.log(error);
            } else {
...
router.post('/api/admin/addMovieInfo', function (req, res) {
  let { movieName, poster, director, actor, long, type, language, publicDate, intro } = req.body;
  ...
  conn.query(sqlStr, [movieName], (error, result, field) => {
    if (error) {
      ...
      if (result[0]) {
        ...
        conn.query(
          sqlStr,
          [movieName, poster, director, actor, long, type, language, publicDate, intro],
          (error, result, field) => {
            if (error) {
              console.log(error);
            } else {
